;$(function () {
    ...
    var addProductToCart = function () {
        ...
        var btn = ${'#addToCart'}convertButtonToLoader(btn, 'btn-primary');
        $.ajax({
    ...
    var calculateCost = function () {
        ...
        var price = parseFloat(priceStr.replace('UAH', ' '));
        ...
        if (count >= 2 && count < 7) {
            if (count >= min && count <= max) {
                var cost = (price - (price * 0.1)) * count;
            ...
            if (count >= min && count <= max) {
                var cost = (price - (price * 0.3)) * count;
                $('#addProductPopup .cost').text(cost);
            } else {
                $('#addProductPopup .count').val(1);
                $('#addProductPopup .cost').text(priceStr);
    ...
    var refreshTotalCost = function () {
        ...
        $('#shoppingCart .item').each(function (index, value) {
            ...
            var price = parseFloat($(value).find('.price').text().replace('UAH', ' '));
            var val = price * count;
            total = total + val;
        });
